...
convoRouter.get(
	...
	checkAuth,
	async (req: Request, res: Response) => {
		const { email } = (req.session as any).user;
		try {
			const sender = await User.findOne({ email: email });
			...
			const newlist = all_convo.map((li) => {
				let reciever;
				if ((li.between[0] as any).email === sender?.email) {
					reciever = li.between[1];
				} else {
					reciever = li.between[0];
				}
...
convoRouter.get(
	...
	async (req: Request, res: Response) => {
		...
		const user = await User.findOne({ email: search });
		if (user) {
			if (user.email != email) return res.json({ status: "ok", user: user });
		} else {
			console.log("not found");
			return res.json({
				status: "Not Found",
...
convoRouter.get(
	...
	async (req: Request, res: Response) => {
		const { convo_id, page_count } = req.query;
		try {
			let page = parseInt(page_count as string);
			let pageSize = 10;
